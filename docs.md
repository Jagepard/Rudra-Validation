...
| public | <br><strong>equals(<strong>$verifiable</strong>, <em>string</em> <strong>$message = 'Values ​​do not match'</strong>)</strong> : <em>[\Rudra\Validation\ValidationInterface](#interface-rudravalidationvalidationinterface)</em><hr /><em>Compares the equivalence of `$verifiable` and `$this->verifiable` values <hr> Сравнивает эквивалентность значений `$verifiable` и `$this->verifiable`</em> |
...
| public | <br><strong>integer(</strong><em>string</em> <strong>$message = 'Number is required'</strong>)</strong> : <em>[\Rudra\Validation\ValidationInterface](#interface-rudravalidationvalidationinterface)</em><hr /><em>Finds whether a `$this->verifiable` is a a numeric string <hr> Проверяет, является ли `$this->verifiable` строкой, содержащей число</em> |
